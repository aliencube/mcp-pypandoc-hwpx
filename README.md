...
로컬 또는 컨테이너 환경에서 파일 시스템에 접근할 수 있을 때 사용합니다.
...
리모트 서버 등 로컬 파일 시스템에 접근할 수 없을 때 사용합니다. 파일 내용을 직접 전달하고 변환 결과를 base64로 반환받습니다.

| 도구                   | 설명                                                                      |
|------------------------|---------------------------------------------------------------------------|
| `convert_docx_content` | base64로 인코딩된 워드(`.docx`) 콘텐츠를 아래아 한글(`.hwpx`)로 변환      |
| `convert_html_content` | HTML 문자열을 아래아 한글(`.hwpx`)로 변환                                 |
| `convert_md_content`   | 마크다운 문자열을 아래아 한글(`.hwpx`)로 변환                             |

#### 매개변수

| 매개변수         | 타입   | 필수 여부 | 설명                                                                                          |
|------------------|--------|-----------|-----------------------------------------------------------------------------------------------|
| `content`        | string | 예        | 원본 콘텐츠 (`.docx`는 base64 인코딩, `.html`/`.md`는 일반 텍스트)                            |
| `filename`       | string | 아니오    | 원본 파일명 (로깅용)                                                                          |
| `reference_hwpx` | string | 아니오    | 스타일 참조용 `.hwpx` 파일의 base64 인코딩 (지정하지 않으면 내장된 `blank.hwpx`를 사용합니다) |
